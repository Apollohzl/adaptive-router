...
const PROMPTS = [
    ...
    },
    {
        ...
        media: 0,
        request:
            "Write a Python function that merges two sorted linked lists, plus pytest tests.",
    },
    {
        label: "DEEP",
        media: 0,
        ...
        request: [
            {
                ...
                content: [
                    ...
                    { type: "input_text", text: "What does this receipt say?" },
                ],
            },
        ],
    },
...
const [catalog, status] = await Promise.all([
    ...
    fetch("https://gen.pollinations.ai/models/status?minutes=30").then((r) =>
        r.json(),
    ),
]);
...
for (const prompt of PROMPTS) {
    ...
    const tier =
        media > 0 && prompt.label === "FAST" ? "BALANCED" : prompt.label;
    const text =
        typeof prompt.request === "string"
            ? prompt.request
    ...
    console.log(
        `tier    : ${tier}${tier === prompt.label ? "" : ` (bumped from ${prompt.label}: images)`}`,
    );
